...
def calculate_diff(data1, data2):
    ...
    - Values are dictionaries describing the change: {'status': 'added'|'deleted'|'changed'|'unchanged'|'nested', 'old_value': ..., 'new_value': ..., 'children': ...}
    """
    if type(data1) != type(data2):
        # For type mismatch, we treat it as a change at the root level (though this case is less common for config files)
    ...
    if isinstance(data1, dict):
        ...
        for key in all_keys:
            ...
            if key not in keys2:
                ...
                diff[key] = {'status': 'deleted', 'old_value': val1, 'new_value': None}
            elif key not in keys1:
                # Key added in file2
                diff[key] = {'status': 'added', 'old_value': None, 'new_value': val2}
            else:
                # Key exists in both
                if isinstance(val1, dict) and isinstance(val2, dict):
                    ...
                    diff[key] = {'status': 'changed', 'old_value': val1, 'new_value': val2}
                else:
                    # Values are identical
                    diff[key] = {'status': 'unchanged', 'old_value': val1, 'new_value': val2}
    else:
        # Handle case where inputs are not dicts (simple values or lists)
        # For simplicity in this tool, we assume config files map to dicts at top level.
        # If they are simple values and equal -> unchanged, else changed.
        if data1 == data2:
            return {'status': 'unchanged', 'old_value': data1, 'new_value': data2}
        else:
            return {'status': 'changed', 'old_value': data1, 'new_value': data2}
